...
Two jobs run without anyone: the nightly reconciliation and the month-end close that follows
it, both runs of the ``reconcile`` pipeline the project's own scheduler starts.
    ...
    return Product(
        ...
        jobs=[
            Job(
                "nightly reconcile",
                pipeline="reconcile",
                does="every night at 02:00, over the day's ledger",
            ),
            Job(
                "month-end close",
                pipeline="reconcile",
                does="on the first of the month, closes the month before",
                after="nightly reconcile",
            ),
        ],
    )
