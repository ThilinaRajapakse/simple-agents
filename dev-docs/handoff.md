...
**v0.1 released as `0.1.0` to `0.1.2`, and `P3-31` closed 2026-09-02.** 4,254 tests. Trajectory format `0.29`, manifest `0.40`, results file `0.29`,
suspension `0.5`, shelf `0.1`, conversation `0.2`, index file `2.0`. Six dogfoods have run; [`runs/dogfood-protocol.md`](runs/dogfood-protocol.md#L1) is the protocol. **Dogfood #6 is `lost-the-plot`, Thilina's own build against the public package, 2026-09-01 to 02**, recorded 2026-09-02 after the fact: [`runs/dogfood-6/findings.md`](runs/dogfood-6/findings.md#L1), 19 candidates, 0 open: the sitting was taken 2026-09-02 and scheduled `P3-72` to `P3-77`. **`P3-72` is built, 2026-09-02** ([build log](build-logs/the-brief-writer-build-log.md#L1)): `simple-agents record` writes every part of the brief, stamped, and FT-44 fails a stamp ahead of the clock. **`P3-73` is built, 2026-09-02** ([build log](build-logs/an-index-that-grows-build-log.md#L1)): `DocumentIndex` grows through `add`, `replace` and `remove`, `NumpyVectors` is the default store with `FaissVectors` behind a new `ann` extra on the CPU or the GPU, the vectors save to a binary file beside the JSON, and the store is on the manifest and in `behaviour_fingerprint` while the corpus size is in neither. Four faults in the shipped code found by the read before the design. **`P3-74` is next.**
...
| The `semantic` extra | `uv sync` without `--extra semantic` prunes it, and one pacing test needs it. **The `ann` extra is the same**: `uv sync --extra semantic --extra mcp --extra ann`, which is what CI runs. Without it `tests/test_faiss_vectors.py` skips rather than fails, so 16 tests go quiet. |
...
| GPU | One RTX 3090, 24GB, about 4GB held by other processes. `--gpu-memory-utilization 0.6` fits `Qwen/Qwen3-1.7B`; 0.85 fails at startup. Port 8000 is taken, so use 8001. **FAISS on the GPU needs its own environment**: `faiss-gpu-cu12` and `faiss-cpu` are two distributions of one module, so `P3-73` verified the GPU store in a second venv holding the GPU wheel and this library. |
