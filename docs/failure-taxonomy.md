...
**Check.** The `drift` array of every entry under the newest agent run's `mcp`. A project that declares no MCP server passes, and so does one whose newest run was replayed, since a replay has nothing to compare. It reads whether the server moved, never whether the move matters: a reworded description may be an improvement, and only the builder can say.
...
**Check.** From stage `ship`: the brief records `confirmed_against`, and it equals the `behaviour_fingerprint` in the newest run under `runs/` whose role is `agent`. Before `ship` the report carries it as a note instead. A project with no answered entry describing the pipeline passes, since there is nothing there to go stale. It reads whether the value was recorded, never whether anyone read the entries, so a project that pastes the fingerprint without reading them passes.

**Failure message.**
> `confirmed_against` in the brief: `<confirmed>`. The newest run under `runs/` was made by `<current>`. These entries describe the pipeline rather than what the builder wants, so each is now recorded agreement to something the code may no longer do: `<due>`. Read each against the code, take any difference to the builder, and record `<current>` in the brief as `confirmed_against`. An entry goes stale when the pipeline changes rather than when a gate is reached, so this is the one check with nothing but a change behind it.
