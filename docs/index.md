...
| `docs/pipeline.md` | The pipeline as a graph, the three node kinds, branching, joining and bounded cycles, suspending a run and resuming it, streaming a node's output, which model a node calls, handing a subtask to a pipeline the model chooses, what a node receives, the output schema and `unknown`, budgets, watching a run in progress, and running the pipeline without a backend | First, and before any node is written |
